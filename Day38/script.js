...
let sjs = 0.9; // slow jump speed (yükseklerdeki atlama hızı katsayısı)
let tw = 17,
  th = 32; // tree width & tree height
...
function agaclariCiz() {
  ...
  for (const x of cizilecekler) agacCiz(x);
}
...
function ziplamayiYonet() {
  ...
  if (jh < mjh * 0.8) jh += jd * js; // yerden yüksekliğini arttır/azalt
  else jh += jd * sjs; // yerden yüksekliğini YAVAŞÇA arttır/azalt
